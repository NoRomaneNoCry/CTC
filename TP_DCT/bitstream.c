...
{
	struct bitstream * struct_retour ;
	ALLOUER(struct_retour, 1);

	struct_retour->ecriture = ('r' == mode[0]) ? Faux : Vrai ;
	struct_retour->nb_bits_dans_buffer = 0;
	
	if(strcmp("-", fichier)  == 0)
		struct_retour->fichier = (struct_retour->ecriture) ? stdout : stdin ;
	else
	{
		struct_retour->fichier = fopen(fichier, mode);
		if(NULL == struct_retour->fichier)
		{
      free(struct_retour);
...
Booleen get_bit(struct bitstream *b)
{
  ...
  retour = prend_bit(b->buffer, b->nb_bits_dans_buffer - 1);
  b->nb_bits_dans_buffer--;
  return retour ;
